    ...
    for (; btr;                                /* Repeat until all data read */
        ...
        if ((fp->fptr % SS(fp->fs)) == 0) {        /* On the sector boundary? */
            ...
            if (cc) {                            /* Read maximum contiguous sectors directly */
                ...
                if (disk_read(fp->fs->drv, rbuff, sect, cc) != RES_OK) ABORT(fp->fs, FR_DISK_ERR);
    ...
    for (; btw;                            /* Repeat until all data written */
        ...
        if ((fp->fptr % SS(fp->fs)) == 0) {    /* On the sector boundary? */
            ...
            if (cc) {                        /* Write maximum contiguous sectors directly */
                if (csect + cc > fp->fs->csize)    /* Clip at cluster boundary */
                    cc = fp->fs->csize - csect;
                if (disk_write(fp->fs->drv, wbuff, sect, cc) != RES_OK) ABORT(fp->fs, FR_DISK_ERR);
#if _FS_MINIMIZE <= 2
#if _FS_TINY
...
/* Start of critical section where any interruption can cause a cross-link */
                        ...
                        if ((dir[DIR_Attr] & AM_DIR) &&
                            djo.sclust != djn.sclust) {    /* Update .. entry in the sub-directory if needed */
...
FRESULT f_mkfs(
    ...
    for (i = 0; i < N_FATS; i++) {        /* Initialize each FAT copy */
        ...
        for (n = 1;
             n < n_fat; n++) {        /* This loop may take a time on FAT32 volume due to many single sector writes */
            if (disk_write(pdrv, tbl, wsect++, 1) != RES_OK)
                return FR_DISK_ERR;
        }
    }
