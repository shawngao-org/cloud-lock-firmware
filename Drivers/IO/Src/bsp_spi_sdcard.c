/**
  ...
  *          +-------------------------+---------------+-------------+
  *          | SD_SPI_CS_PIN           |   ChipSelect  |    8        |
  *          | SD_SPI_MOSI_PIN / MOSI  |   DataIn      |    7        |
  *          |                         |   GND         |     (0 V)   |
  *          |                         |   VDD         |     (3.3 V) |
  *          | SD_SPI_SCK_PIN / SCLK   |   Clock       |    5        |
  *          |                         |   GND         |     (0 V)   |
  *          | SD_SPI_MISO_PIN / MISO  |   DataOut     |    6        |
  *          +-------------------------+---------------+-------------+
  ******************************************************************************
  * @attention
  *
...
void SD_DeInit(void) {
    ...
    __HAL_SPI_DISABLE(&hspi1); /*!< SD_SPI disable */
    SPI_I2S_DeInit();   /*!< DeInitializes the SD_SPI */
...
//  GPIO_PinRemapConfig(GPIO_Remap_SPI3, DISABLE);
    ...
    HAL_GPIO_Init(SD_SPI_SCK_GPIO_PORT, &GPIO_InitStructure);

    /*!< Configure SD_SPI pins: MISO */
    GPIO_InitStructure.Pin = SD_SPI_MISO_PIN;
    HAL_GPIO_Init(SD_SPI_MISO_GPIO_PORT, &GPIO_InitStructure);

    /*!< Configure SD_SPI pins: MOSI */
    GPIO_InitStructure.Pin = SD_SPI_MOSI_PIN;
    HAL_GPIO_Init(SD_SPI_MOSI_GPIO_PORT, &GPIO_InitStructure);

    /*!< Configure SD_SPI_CS_PIN pin: SD Card CS pin */
    GPIO_InitStructure.Pin = SD_CS_PIN;
    HAL_GPIO_Init(SD_CS_GPIO_PORT, &GPIO_InitStructure);
...
//    }
//
//    __HAL_SPI_ENABLE(&SPI_InitStructure); /*!< SD_SPI enable */
...
uint8_t SD_WriteByte(uint8_t Data) {
    ...
    while (SPI_I2S_GetFlagStatus(hspi1.Instance, SPI_I2S_FLAG_TXE) == RESET) {
    }
    ...
    while (SPI_I2S_GetFlagStatus(hspi1.Instance, SPI_I2S_FLAG_RXNE) == RESET) {
    }
...
uint8_t SD_ReadByte(void) {
    ...
    /*!< Send the byte */
    SPI_I2S_SendData(hspi1.Instance, SD_DUMMY_BYTE);

    /*!< Wait until a data is received */
    while (SPI_I2S_GetFlagStatus(hspi1.Instance, SPI_I2S_FLAG_RXNE) == RESET) {
    }
    /*!< Get the received data */
    Data = SPI_I2S_ReceiveData(hspi1.Instance);
